...
public  class Rubrica { 
    ...
    public void scriviOutput() {
        ...
        System.out.println("Lunghezza Tot = " + contatti.length);
    ...
    private int cercaIndice(String n) {
        int i=0;
        if(Character.getNumericValue(n.charAt(0)) > 77 || Character.getNumericValue(n.charAt(0)) > 109){
            i = (contatti.length-1/2)-1;
        }
    ...
    public boolean aggiungi_Ord(String n, String e) {
        if (presente(n)) 
            return false;      // rubrica contiene n: fallimento
        if (piena()) 
            raddoppio();
        ...
        //aggiungo il nuovo contatto nella prima posizione disponibile
        if(numContatti > 0){
            int new_Index = ordinamento(n);
            Contatto temp;
            for(int i = contatti.length-1; i > new_Index; i--){
                contatti[i] = contatti[i-1];
            }
            contatti[new_Index] = new Contatto(n,e);
        }
        else{
            contatti[numContatti] = new Contatto(n,e);
        }
    ...
    private void raddoppio(){
        Contatto[] vettext = new Contatto[contatti.length*2];

        for(int i = 0; i < contatti.length; i++){
            vettext[i] = contatti[i];
        }
        contatti = vettext;
    }
    ...
    public int ordinamento(String n){
        boolean flag = false;
        for(int i = 0; i < contatti.length; i++){
            if(contatti[i] == null)
                return i;
            else{
                if(Character.getNumericValue(contatti[i].getNome().charAt(0)) > Character.getNumericValue(n.charAt(0)))
                    return i;
                if(Character.compare(contatti[i].getNome().charAt(0), n.charAt(0)) == 0)
                    for(int j = i; j < n.length() && j < contatti[i].getNome().length(); j++)
                        if(Character.getNumericValue(contatti[i].getNome().charAt(j)) > Character.getNumericValue(n.charAt(j))){
                            flag = true;
                            return i;
                        }
