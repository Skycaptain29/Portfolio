...
def time_one(filter_function: Callable, *arguments, calls: int = 3) -> float:
    ...
    times = np.subtract(etimes, stimes)
    return np.sum(times)/calls
...
def make_reports(filename: str = "test/rain.jpg", calls: int = 3):
    ...
    Args:
        ...
        calls (int): the number of times we want to run the test
    """
    ...
    print(f"Timing performed using {filename}: shape {image.shape}")
    file.write(f"Timing performed using {filename}: shape {image.shape}\n")
